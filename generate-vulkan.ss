...
(let* ((in (open-file-input-port "res/vulkan.fasl"))
       (out (open-output-file "res/vulkan.ss" '(truncate)))
       ...
       (display (lambda (sexp)
                  ...
                  (if (and (string-prefix? "define" (symbol->string (car sexp)))
                           (string-prefix? "vk:" (symbol->string (cadr sexp))))
